...
type PredefinedDrink = Pick<
  Drink,
  "kategorie" | "sorte" | "geschmack" | "limitiert"
>;
...
const PREDEFINED_BY_ID = new Map(
  PREDEFINED_DRINKS.map((drink) => [drink.id, drink]),
);
const PREDEFINED_ID_BY_LOOKUP = new Map(
  PREDEFINED_DRINKS.map((drink) => [
    predefinedLookupKey(drink.kategorie, drink.sorte),
    drink.id,
  ]),
);
...
export function loadDrinks(): Drink[] {
  try {
    ...
    if (Array.isArray(parsed))
      return mergeById(PREDEFINED_DRINKS, normalizeDrinks(parsed));
...
function normalizeDrink(value: unknown): Drink {
  ...
  if (typeof drink.sorte !== "string" || typeof drink.kategorie !== "string") {
    throw new Error("Ungültiges Format: Drink-Eintrag fehlerhaft.");
  }

  const kategorie = normalizeKategorie(drink.kategorie);
  const predefinedId = PREDEFINED_ID_BY_LOOKUP.get(
    predefinedLookupKey(kategorie, drink.sorte),
  );
  return {
    id:
      predefinedId ??
      (typeof drink.id === "string" && drink.id ? drink.id : makeId()),
    kategorie,
    sorte: drink.sorte,
...
function normalizeBewertung(bewertung: unknown): Bewertung | null {
  if (bewertung === null || bewertung === undefined || bewertung === "")
    return null;
  if (
    bewertung === "S" ||
    bewertung === "A" ||
...
function isExportPayload(data: unknown): data is ExportPayload {
  return (
    typeof data === "object" &&
    data !== null &&
    (data as Partial<ExportPayload>).version === 1 &&
    Array.isArray((data as Partial<ExportPayload>).drinks)
  );
}
